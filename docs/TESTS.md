...
* **`tests/d3d11on12shaderlifecycle.c`:** the same treatment for the owned shader lifecycle, and the suite that pins *which* driver entry creates a shader. `tests/d3d11on12mockdriver.c` reproduces the pinned driver's actual slot population — `pfnCalcPrivateShaderSize`, `pfnDestroyShader`, `pfnVsSetShader` and `pfnPsSetShader` filled, `pfnCreateVertexShader` and `pfnCreatePixelShader` deliberately left null — and publishes creation only through an `ID3D11On12DDIDevice` vtable at the head of the private device block. A host that called the table slots would fault here rather than against the real driver. The suite also asserts the caller's bytecode container is passed through by address and then overwrites it before binding, so host-side retention would surface; refuses cross-stage binds, which the driver itself would accept as well-formed; and closes a device with shaders still outstanding, requiring the host to destroy them through the driver before the device goes away and to leave the caller's stale handles inert rather than double-freeable.
