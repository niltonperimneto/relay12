...
**0003, winemac: let a process render into another process's window.** `macdrv_vulkan_surface_create` gives up when `get_win_data` fails, which it always does for a window owned by another process, so `CreateSwapChainForHwnd` returns `E_INVALIDARG` and chromium's gpu process dies with `eglCreateWindowSurface: EGL_BAD_ALLOC`. that is what makes steam's window black here. an NSView can only be reached from the process that made it, so the rendering process now publishes its metal layer to the window server with a `CAContext` and sends the resulting context id to the owning process, which displays it with a `CALayerHost` parented in its client view. this is the same route chromium's own gpu process takes on macos. all three symbols are undocumented and are looked up at runtime, so a system without them just falls back to the old behaviour. upstream wine 11 has the same gap, it only fails quietly: the surface is created and then never parented, so the window is black with no error.
